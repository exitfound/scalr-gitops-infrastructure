...
| Файл | Что менять |
|------|-----------|
| `gke/versions.tf` | GCS bucket name |
...
`gke` и `scalr-admin` независимы друг от друга — применяются одновременно.
...
Нужно запомнить значения из `agents.main.scalr_agent_gsa_email` и `eso_gsa_email` — понадобятся в Шаге 8.
...
### Шаг 7 — Настроить kubectl на кластер
...
### Шаг 8 — Создать serviceaccount.yaml с Workload Identity аннотацией
...
### Шаг 9 — terraform apply: fluxcd-bootstrap
...
### Шаг 10 — Дождаться автоматического деплоя
...
Изменения применяются к существующему кластеру. `deletion_protection = true` предотвращает случайное удаление.
...
`deletion_protection = true` по умолчанию. Чтобы удалить:
```bash
