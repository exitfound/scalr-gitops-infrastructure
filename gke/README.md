...
|--------|----------|----------------|
| `cluster_name` | Имя кластера | `fluxcd-bootstrap` → `terraform_remote_state` |
...
| `cluster_location` | Регион кластера | `fluxcd-bootstrap` → `terraform_remote_state` |
...
`fluxcd-bootstrap` читает `cluster_name` и `cluster_location` через `terraform_remote_state` — копировать вручную не нужно.
...
`cluster_name` и `cluster_location` читаются автоматически через `terraform_remote_state` — ничего копировать не нужно.

В `fluxcd/fluxcd-bootstrap/envs/scalr.tfvars` по-прежнему нужно задать `gcp_project_id` (используется провайдером Google до инициализации data sources).
